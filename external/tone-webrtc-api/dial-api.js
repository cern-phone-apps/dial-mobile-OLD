...
export class Dial {
  ...
  addTrackListener() {
    this.session.on("trackAdded", () => {
      console.log("RECEIVED TRACKADDED");
      console.log(event);
      ...
      if (sdh === undefined) {
        ...
        pc.getRemoteStreams().forEach(function(sender) {
          let audioTracks = sender.getAudioTracks();
          console.log(audioTracks);
          // console.log(sender.track);
          remoteStream.addTrack(audioTracks[0]);
        });
        //
        // //Gets remote tracks
        // var remoteStream = new MediaStream();
        // console.log(`RemoteStream = new MediaStream()`);
  ...
  setSession(session) {
    ...
    // session.on("addStream", (event) => {console.log(event)});
    // session.on("getDescription", (event) => {console.log(event)});
    // session.on("addTrack", (event) => {console.log(event)});

    session.on("userMedia", function(stream) {
      console.log("USER MEDIA EVENT");
      console.log(stream);
    });
    ...
    //       "userMediaFailed",
    //       function() {
    //         console.error(`User Media has failed`);
    //         this.endCleanup();
